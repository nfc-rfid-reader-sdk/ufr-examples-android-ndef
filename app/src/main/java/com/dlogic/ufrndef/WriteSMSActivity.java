...
public class WriteSMSActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LinearLayout bigLayout = findViewById(R.id.writeSMSLayout);
        LinearLayout storageLayout = findViewById(R.id.writeSMSStorage);

        if (device == 0) {
            storageLayout.setVisibility(View.GONE);
            ViewGroup.LayoutParams params = bigLayout.getLayoutParams();
            final float scale = getResources().getDisplayMetrics().density;
            params.height = (int) (240 * scale);
            bigLayout.setLayoutParams(params);
        } else {
            bigLayout.setVisibility(View.VISIBLE);
        }

        LinearLayout back = findViewById(R.id.back_from_sms);
        ...
        Spinner spnStoreInto = findViewById(R.id.sms_store_into);
        ...
        Button btnWriteSMS = findViewById(R.id.buttonWriteSMS);
        btnWriteSMS.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                EditText phone = findViewById(R.id.SMSphoneET);
                EditText message = findViewById(R.id.SMSmessageET);
                ...
                status = uFCoder.WriteNdefRecord_SMS(storage, phoneStr, messageStr);
